...
function CouchDBServer(config, callback) {
    ...
    const CouchDBEnclaveFacade = require("loki-enclave-facade");
    ...
    try {
        ...
        for (let entry of folderContent) {
            if (entry.isDirectory()) {
                ...
                enclaves[enclaveName] = CouchDBEnclaveFacade.createCouchDBEnclaveFacadeInstance(path.join(lightDBStorage, enclaveName, DATABASE));
    ...
    function registerEndpoints() {
        ...
        server.put(`/createDatabase/:dbName`, function (req, res) {
            ...
            fsModule.mkdir(storage, {recursive: true}, (err) => {
                ...
                enclaves[dbName] = CouchDBEnclaveFacade.createCouchDBEnclaveFacadeInstance(path.join(storage, DATABASE));
                res.statusCode = 201;
                res.end();
            })
        });
